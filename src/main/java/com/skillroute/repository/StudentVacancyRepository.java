...
public interface StudentVacancyRepository extends JpaRepository<StudentVacancy, StudentVacancyId> {
    ...
    @Query("SELECT sv FROM StudentVacancy sv JOIN FETCH sv.student JOIN FETCH sv.vacancy v WHERE v.company.id = :companyId")
    List<StudentVacancy> findAllByCompanyId(@Param("companyId") Long companyId);
}
